...
function getPolynom(...args) {
  const func = (x) => {
    ...
    return array.reduce((acc, el, i, arr) => acc + el * (x ** (arr.length - 1 - i)), 0);
  };
  return func;
}
...
function logger(func, logFunc) {
  const fn = (...args) => {
    const array = JSON.stringify(args).split('');
    array.pop();
    array.shift();
    logFunc(`${func.name}(${array.join('')}) starts`);
    const res = func(...args);
    logFunc(`${func.name}(${array.join('')}) ends`);
    return res;
  };
  return fn;
}
