...
function isCreditCardNumber(ccn) {
  let arrCard = String(ccn).split('').reverse();
  arrCard = arrCard.map((el, i) => {
    if (i % 2 !== 0) {
      let elem = Number(el) * 2;
  ...
  const sum = arrCard.reduce((acc, el) => acc + el, 0);
  if (sum % 10 === 0) {
    return true;
  }
  return false;
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < arrStr.length; i += 1) {
    if (brackets.map((el) => el.open).includes(arrStr[i])) {
      stack.push(arrStr[i]);
    } else {
      const stackElement = stack.pop();
      if (!stackElement || brackets.find((el) => el.open === stackElement).close !== arrStr[i]) {
        return false;
      }
    }
  }
...
function getCommonDirectoryPath(pathes) {
  const splittedPathes = pathes.map((el) => el.split('/'));
  const maxLength = Math.max(...splittedPathes.map((el) => el.length));
  const result = [];

  for (let i = 0; i < maxLength; i += 1) {
    const newElement = splittedPathes[0][i];
    if (splittedPathes.every((el) => el[i] === newElement)) {
...
function getMatrixProduct(m1, m2) {
  ...
  for (let i = 0; i < m1.length; i += 1) {
    for (let j = 0; j < m2.length; j += 1) {
      const number = m1[i][0] * m2[0][j] + m1[i][1] * m2[1][j] + m1[i][2] * m2[2][j];
      if (number) {
        stack[j] = number;
      }
    }
  ...
  for (let i = 0; i < position.length; i += 1) {
    for (let j = 0; j < position[i].length; j += 1) {
      ...
      if (position[0][j] === position[1][j]
        ...
        && position[0][j] && position[1][j] && position[2][j]) {
        return position[0][j];
      }
